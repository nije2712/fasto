...
(*
  ...
  | evalExp ( Not(e1, pos), vtab, ftab ) =
        let val res1   = evalExp(e1, vtab, ftab)
        in  evalBinopNum(op not, res1, pos)
        end
*)
  ...
  | evalExp ( Greater(e1, e2, pos), vtab, ftab ) =
        let val r1 = evalExp(e1, vtab, ftab)
            val r2 = evalExp(e2, vtab, ftab)
        in  evalRelop(op >, r1, r2, pos)   (* > *)
        end
...
and evalFunArg (FunName fid, vtab, ftab, callpos) =
  ...
  | evalFunArg (Lambda (rettype, params, body, fpos), vtab, ftab, callpos) =
     let
        val f = FunDec ("anonymous", rettype, params, body, fpos)
     in
        (fn aargs => callFunWithVtable(f, aargs, vtab, ftab, callpos), rettype)
     end
   (* TODO TASK 3:
