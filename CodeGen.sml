...
(* All - lavet i øvelsestimen
   
*)

    | All (arr, pos) =>
        let val arr_reg = newName "arr_reg" (* peger på pointer til det nuværende element *)
        val arr_len = newName "arr_len"
        val arr_end = newName "arr_end"
        val elem_reg = newName "elem_reg"
        val should_we_test = newName "should_we_test"

        val loop_begin = newName "all_begin"
        val loop_end = newName "all_end"

        val arr_code = compileExp arr vtable arr_reg
        val code =
            [ Mips.LI (place, "1")
            , Mips.LW (arr_len, arr_reg, "0")
            , Mips.ADDI (arr_reg, arr_reg, "4")
            , Mips.ADD (arr_end, arr_reg, arr_len)
            , Mips.LABEL loop_begin
            ,   Mips.SLT (should_we_test, arr_reg, arr_end)
            ,   Mips.BEQ (should_we_test, "0", loop_end)
            ,       Mips.LB (elem_reg, arr_reg, "0")
            ,       Mips.AND (place, place, elem_reg)
            ,       Mips.ADDI (arr_reg, arr_reg, "1")
            ,       Mips.BNE (place, "0", loop_begin)
            , Mips.LABEL loop_end 
            ]
        in arr_code @ code
        end
