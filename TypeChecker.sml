...
and checkExp ftab vtab (exp : In.Exp)
  = case exp of
    ...
    | In.Negate (e, pos)
      => let val (e_type, e_dec) = checkExp ftab vtab e
         in (Int,
    ...
    | In.Reduce (f, n_exp, arr_exp, _, pos)
     ...
     | In.All (arr, pos) =>
        let val (ty, arr') = checkExp ftab vtab arr
        in if ty <> Array Bool
            then raise Error("Arugmment to all has type " ^ ppType ty, pos)
            else (Bool,
                  Out.All (arr', pos))
        end 
